...
module XBar
  module Examples
    ...
    # required.
    #
    # At the end, the operations that are invoked on each shard together
    # with the number of times that the operation was invoked on the shard
    # are displayed.
    #
    module Basic
      ...
      case method
      when :migrations
        puts "Using a migration to create initial table(s)"
        MIGRATIONS_ROOT = File.expand_path(File.join(File.dirname(__FILE__), 'migrations'))
        version = 1
        ActiveRecord::Migrator.run(:up, MIGRATIONS_ROOT, version)
      when :establish_connection
        puts "Using raw SQL to to create the initial tables(s)."
      ...
      when :create_table_on_connection
        puts "Using a method on the database adapter to create initial tables."
        [:deli, :bakery, :produce].each do |shard|
